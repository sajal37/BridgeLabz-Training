using Google_Classroom.Week_1.Day_4.Day_8_Problem_Statement;
using Google_Classroom.Week_1.Day_3._04_C__Methods_Level_3_Practice_Programs;
using Google_Classroom.Week_1.Day_3._03_C__Methods_Level_2_Practice_Programs;
using Google_Classroom.Week_1.Day_3._02_C__Methods_Level_1_Practice_Programs;
using Google_Classroom.Week_1.Day_2._03_C__Arrays_Level_2_Lab_Practice;
using Google_Classroom.Week_1.Day_2._02_C__Arrays_Level_1_Lab_Practice;
using Google_Classroom.Week_1.Day_2._04_C__Control_Flow_Level_3_Lab_Practice;
using Google_Classroom.Week_1.Day_2._03_C__Control_Flow_Level_2_Lab_Practice;
using Google_Classroom.Week_1.Day_2._02_C__Control_Flow_Level_1_Lab_Practice;
using Google_Classroom.Practice_Problem_3;
using Google_Classroom.Practice_Problem_2;
using Google_Classroom.Practice_Problem_1;

// Day 4: Day 8 Problem Statement
//new Count_Vowels_and_Consonants().Demo();
//new Reverse_a_String().Demo();
//new Palindrome_String_Check().Demo();
//new Remove_Duplicates_from_a_String().Demo();
//new Find_the_Longest_Word_in_a_Sentence().Demo();
//new Find_Substring_Occurrences().Demo();
//new Toggle_Case_of_Characters().Demo();
//new Compare_Two_Strings().Demo();
//new Find_the_Most_Frequent_Character().Demo();
//new Remove_a_Specific_Character_from_a_String().Demo();
//new Anagram_Check().Demo();
new Replace_Word_in_Sentence().Demo();

// Day 3: 04 C# Methods Level 3 Practice Programs
//new Football_Team_Heights().Demo();
//new Number_Checker_Utility_Part_1().Demo();
//new Number_Checker_Utility_Part_2().Demo();
//new Number_Checker_Utility_Part_3().Demo();
//new Number_Checker_Utility_Part_4().Demo();
//new Number_Checker_Utility_Part_5().Demo();
//new Generate_Unique_OTP().Demo();
//new Display_Month_Calendar().Demo();
//new Euclidean_Distance_And_Line_Equation().Demo();
//new Collinear_Points_Check().Demo();
//new Zara_Employee_Bonus_Generator().Demo();
//new Student_Scorecard_Generator().Demo();
//new Matrix_Operations_Utility().Demo();

// Day 3: 03 C# Methods Level 2 Practice Programs
//new Factors_Analysis().Demo();
//new Sum_Of_Natural_Numbers_Recursive().Demo();
//new Leap_Year_Check_Method().Demo();
//new Unit_Converter_Distance().Demo();
//new Unit_Converter_Extended().Demo();
//new Unit_Converter_Full().Demo();
//new Student_Vote_Checker().Demo();
//new Youngest_And_Tallest_Friends_Method().Demo();
//new Number_Analysis_Method().Demo();
//new BMI_2D_Array_Method().Demo();
//new Quadratic_Roots().Demo();
//new Random_Array_Min_Max_Average().Demo();

// Day 3: 02 C# Methods Level 1 Practice Programs
//new Simple_Interest().Demo();
//new Handshake_Calculation().Demo();
//new Divide_Chocolates().Demo();
//new Spring_Season_Check().Demo();
//new Trigonometric_Functions().Demo();
//new Smallest_And_Largest_Of_Three().Demo();
//new Check_Number_Sign().Demo();
//new Wind_Chill_Calculation().Demo();
//new Athlete_Triangular_Park_Rounds().Demo();
//new Sum_Of_Natural_Numbers_Method().Demo();
//new Remainder_And_Quotient().Demo();

// Day 2: 03 C# Arrays Level 2 Lab Practice
//new Employee_Bonus_Zara().Demo();
//new Youngest_And_Tallest_Friends_Array().Demo();
//new Largest_And_Second_Largest_Digit_Fixed_Array().Demo();
//new Largest_And_Second_Largest_Digit_Dynamic_Array().Demo();
//new Reverse_Number_Array().Demo();
//new BMI_Array_All_Persons().Demo();
//new BMI_2D_Array_All_Persons().Demo();
//new Student_Grade_Array().Demo();
//new Student_Grade_2D_Array().Demo();
//new Digit_Frequency_Array().Demo();

// Day 2: 02 C# Arrays Level 1 Lab Practice
//new Student_Voting_Eligibility_Array().Demo();
//new Number_Analysis_Array().Demo();
//new Multiplication_Table_Array().Demo();
//new Sum_Until_Zero_Or_Negative_Array().Demo();
//new Multiplication_Table_6_To_9_Array().Demo();
//new Mean_Height_Football_Team().Demo();
//new Odd_And_Even_Arrays().Demo();
//new Factors_Array().Demo();
//new Copy_2D_Array_To_1D_Array().Demo();
//new FizzBuzz_Array().Demo();

// Day 2: 04 C# Control Flow Level 3 Lab Practice
//new Check_Armstrong_Number().Demo();
//new Count_Digits().Demo();
//new Check_Harshad_Number().Demo();
//new Check_Abundant_Number().Demo();
//new Day_Of_Week().Demo();
//new Calculator_Switch_Case().Demo();

// Day 2: 03 C# Control Flow Level 2 Lab Practice
//new Leap_Year_Multiple_If().Demo();
//new Leap_Year_Single_If().Demo();
//new Grade_Calculation().Demo();
//new Prime_Number_Check().Demo();
//new FizzBuzz_For().Demo();
//new FizzBuzz_While().Demo();
//new BMI_Calculation().Demo();
//new Youngest_And_Tallest_Friends().Demo();
//new Greatest_Factor().Demo();
//new Power_Of_Number().Demo();
//new Factors_Of_Number().Demo();
//new Multiples_Below_100().Demo();

// Day 2: 02 C# Control Flow Level 1 Lab Practice
//new Check_Divisible_By_5().Demo();
//new Check_Smallest_Of_Three().Demo();
//new Check_Largest_Of_Three().Demo();
//new Sum_Of_Natural_Numbers_Formula().Demo();
//new Voting_Eligibility().Demo();
//new Check_Positive_Negative_Zero().Demo();
//new Spring_Season().Demo();
//new Rocket_Launch_Countdown_While().Demo();
//new Rocket_Launch_Countdown_For().Demo();
//new Sum_Until_Zero_While().Demo();
//new Sum_Until_Zero_Or_Negative_Break().Demo();
//new Sum_Natural_Numbers_While_Comparison().Demo();
//new Sum_Natural_Numbers_For_Comparison().Demo();
//new Factorial_While().Demo();
//new Factorial_For().Demo();
//new Odd_Even_Numbers().Demo();
//new Employee_Bonus().Demo();
//new Multiplication_Table_6_To_9().Demo();

// Practice Problem 3
//new Quotient_and_Remainder().Demo();
//new Int_Operation().Demo();
//new Double_Opt().Demo();
//new Celsius_to_Fahrenheit().Demo();
//new Fahrenheit_to_Celsius().Demo();
//new Total_Income().Demo();
//new Swap_Two_Numbers().Demo();
//new Sample_Program_2_Input().Demo();
//new Triangle_Park_Rounds().Demo();
//new Chocolate_Distribution().Demo();
//new Simple_Interest().Demo();
//new Pounds_To_Kg().Demo();

// Practice Problem 2
//new Harry_s_Age().Demo();
//new Sam_Average_PCM().Demo();
//new Convert_10_8_Km_To_Miles().Demo();
//new Profit_and_Loss().Demo();
//new Pens_Distribution().Demo();
//new University_Discount_Fee().Demo();
//new Volume_of_Earth().Demo();
//new Km_To_Miles_User_Input().Demo();
//new Discounted_Fee_UserInput().Demo();
//new Height_Cm_To_Feet_Inches().Demo();
//new Basic_Calculator_Float().Demo();
//new Triangle_Area_Inches_Cm().Demo();
//new Square_Side_From_Perimeter().Demo();
//new Feet_To_Yards_Miles().Demo();
//new Total_Price().Demo();
//new Max_Handshakes().Demo();

// Practice Problem 1
//new Welcome_to_Bridgelabz_().Demo();
//new Add_Two_Numbers().Demo();
//new Celsius_to_Fahrenheit_Conversion().Demo();
//new Area_of_a_Circle().Demo();
//new Volume_of_a_Cylinder().Demo();
//new Calculate_Simple_Interest().Demo();
//new Perimeter_of_a_Rectangle().Demo();
//new Power_Calculation().Demo();
//new Calculate_Average_of_Three_Numbers().Demo();
//new Convert_Kilometers_to_Miles().Demo();
