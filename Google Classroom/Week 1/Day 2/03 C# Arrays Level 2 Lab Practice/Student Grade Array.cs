...
{
    internal class Student_Grade_Array
    {
        public void Demo()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                Console.WriteLine($"Student {i + 1} - Marks: P={physics[i]}, C={chemistry[i]}, M={maths[i]} | Percentage: {percentages[i]}% | Grade: {grades[i]} | Remarks: {remarks[i]}");
            }
        }
    }
}
